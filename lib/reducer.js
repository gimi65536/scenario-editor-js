...
export default function reducer(draft, action_or_array){
	if(Array.isArray(action_or_array)){
		const actions = action_or_array;
		for (const action of actions) {
			doAction(draft, action);
		}
	}else{
		doAction(draft, action_or_array);
	}
}

function doAction(draft, action){
	switch (action.type) {
		case 'edit_dialogue':
		...
		case 'edit_speakers':
			...
			break;
		case 'batch_add_below':
			editFunctions.batchAddBelow(draft, action.below, action.lines, action.strip);
			break;
		case 'move_character':
		...
		case 'delete_character':
			...
			break;
		case 'edit_character_info':
			editFunctions.editCharacterInfo(draft, action.uuid, action.info)
			break;
		case 'edit_title':
			editFunctions.editTitle(draft, action.title);
			break;
		case 'edit_info':
			editFunctions.editInfo(draft, action.info);
			break;
		default:
