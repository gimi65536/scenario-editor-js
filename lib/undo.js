...
export default function useUndoReducer(immerReducer, initialState, stackCapacity = defaultStackCapacity, shrinkTo = defaultShrinkTo) {
	...
	const _setter = useCallback((action, toState) => {
		...
		if(redoPatches.length + undoPatches.length === 0){
			return;
		}
...
export function useUndo(initialState, stackCapacity = defaultStackCapacity, shrinkTo = defaultShrinkTo){
	const [
		...
		setUnmodified,
		reset
	] = useUndoReducer((draft, action) => { }, initialState, stackCapacity, shrinkTo);
