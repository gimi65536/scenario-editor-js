 ...
 * @property {Object.<String, {components: Array<MacroComponent>, speakers_list: Array<String>, speaker?: String}>} dialogues.reference
 */
...
export const editFunctions = {
	...
	moveDialogue: (s, from, to) => {
		...
		if (hydratedLevel(s) >= 2) {
			const uuid_culprit = s.dialogues.order[to];
			let set = s.__hydration.dialogueToIndex.get(uuid_culprit);
			set.delete(from);
			set.add(to);

			// All victims
			if(from < to){
				// All elements in [from, to)
				for(let i = from; i < to; i++){
					const uuid = s.dialogues.order[i];
					set = s.__hydration.dialogueToIndex.get(uuid);
					set.delete(i + 1);
					set.add(i);
				}
			}else if(from > to){
				// All elements in (to, from]
	 ...
	 * Soft-need (1)
	 * 
	 * The reason not make this hard-need is that we still need to
	 * convert `characters` into set no matter if scenario is hydrated.
	 * @param {Scenario} s
	 * @param {Set<String>} uuids
	...
	editSpeakers: (s, uuids, characters) => {
		for(const uuid of uuids){
			...
			dialogue.speakers_list = characters;
			// Hydrated
			if(isHydrated(s)){
				const old_speakers_set = ImmutableSet(old_speakers_list);
				const new_speakers_set = ImmutableSet(characters);
				// Dropped
	...
	moveCharacter: (s, from, to) => {
		...
		if (hydratedLevel(s) >= 2) {
			const uuid_culprit = s.characters.order[to];
			s.__hydration.characterToIndex.set(uuid_culprit, to);
			...
			if (from < to) {
				...
				for (let i = from; i < to; i++) {
					...
					s.__hydration.characterToIndex.set(uuid, i);
				}
			} else if (from > to) {
				// All elements in (to, from]
				for (let i = to + 1; i <= from; i++) {
	...
	addCharacter: (s) => {
		...
		if (hydratedLevel(s) >= 2) {
			s.__hydration.characterToIndex.set(newUUID, s.characters.order.length - 1);
		}
	},
	/**
	 * Hard-need 1 (2)
	...
	deleteCharacter: (s, i) => {
		...
		if (hydratedLevel(s) >= 2) {
			for (let j = s.characters.order.length - 1; j >= i; j--) {
				const uuid = s.characters.order[j];
				s.__hydration.characterToIndex.set(uuid, j);
			}
		}
	},
}