...
export default function useUndoReducer(immerReducer, initialState, {
	...
	const _setter = useCallback((action, toState) => {
		...
		if (skipNoChange && redoPatches.length + undoPatches.length === 0){
			return;
		}
	...
	}, [state, skipNoChange, stackPointer, stackCapacity, ahead, immerReducer, patchesStack, shrinkTo]);
...
export function useUndo(initialState, kwargs = {}){
	const [
		state,
		isModified,
		undo,
		...
		setUnmodified,
		reset
	] = useUndoReducer((draft, action) => { }, initialState, kwargs);
